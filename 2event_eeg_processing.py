...
def write_csv(data):
    with open('2summary_events.csv', 'a') as outfile:
...
if not os.path.exists('2summary_events.csv'):
    write_csv(cols)
...
list_fn = glob.glob('data/slide2/long/*.set')

######################################
# 2 - process each subject
for f in tqdm.tqdm(list_fn):
    # check if file was processed already, otherwise skip
    if os.path.exists('2summary_events.csv'):
        temp = pd.read_csv('2summary_events.csv')
    ...
    # load epochs if raw file
    try:
        raw = mne.io.read_epochs_eeglab(f)
        events_from_annot = raw.events
        event_dict = raw.event_id
        sl2_ev = [v for k, v in event_dict.items() if 'sl2' in k.split('###')[0]]
        sl02_ev = [v for k, v in event_dict.items() if 'sl02' in k.split('###')[0]]
        sl2 = sl2_ev + sl02_ev

        # init_idx = [np.where(events_from_annot[:, 2] == _)[0][0] for _ in sl2]
        # sl2_samp = events_from_annot[init_idx, 0].tolist()
    ...
    for idx in sl2:
        sl2_raw = raw_ica[idx].crop(tmin=0.1, tmax=0.2)
        ...
        all_data['bodyposition'] = subj_data.split('_')[2]
